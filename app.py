...
st.sidebar.divider()
input_currency = st.sidebar.selectbox(
    "Your data's currency", ["USD ($)", "RMB (¥)"], index=0,
    help="What currency are the order_value amounts in your uploaded CSV?",
)
exchange_rate = st.sidebar.number_input(
    "Exchange rate (1 USD = ? RMB)", min_value=0.01, value=7.15, step=0.01, format="%.2f",
    help="Used to show CLV in both currencies. Update to the current rate as needed "
         "(around ¥7.1-7.2 per USD as of mid-2026).",
)
...
with st.spinner("Building RFM summary and fitting models..."):
    ...
    if input_currency.startswith("USD"):
...
k3.metric(f"Avg predicted {clv_horizon}mo CLV (USD)", f"${summary['clv_usd'].mean():,.2f}")
k4.metric(f"Avg predicted {clv_horizon}mo CLV (RMB)", f"¥{summary['clv_rmb'].mean():,.2f}")
k5.metric("Total predicted CLV (USD)", f"${summary['clv_usd'].sum():,.0f}")
...
with tab_table:
    ...
    filter_currency = st.radio("Filter by", ["USD ($)", "RMB (¥)"], index=0, horizontal=True)
    if filter_currency.startswith("USD"):
        max_val = float(summary["clv_usd"].max())
        min_clv = st.slider("Minimum CLV filter (USD)", 0.0, max_val, 0.0)
        filtered = summary[summary["clv_usd"] >= min_clv]
    else:
        max_val = float(summary["clv_rmb"].max())
        min_clv = st.slider("Minimum CLV filter (RMB)", 0.0, max_val, 0.0)
        filtered = summary[summary["clv_rmb"] >= min_clv]

    st.caption(f"Showing {len(filtered):,} of {len(summary):,} customers "
               f"· exchange rate used: 1 USD = ¥{exchange_rate:.2f}")
    st.dataframe(
    ...
    st.download_button(
        "Download CLV table as CSV (both currencies)",
